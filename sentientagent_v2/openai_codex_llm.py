...
DEFAULT_ORIGINATOR = "sentientagent_v2"
...
def _get_codex_token() -> Any:
    ...
    if not token or not getattr(token, "access", ""):
        raise RuntimeError(
            "OpenAI Codex OAuth token missing. Run: sentientagent_v2 provider login openai-codex"
        )
...
def _build_headers(account_id: str, access_token: str) -> dict[str, str]:
    ...
    return {
        ...
        "User-Agent": "sentientagent_v2 (python)",
...
def _friendly_error(status_code: int, body_text: str) -> str:
    ...
    if status_code == 401:
        return "Codex authentication failed. Please re-run: sentientagent_v2 provider login openai-codex"
    if status_code == 429:
