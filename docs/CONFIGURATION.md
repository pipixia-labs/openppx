...
Docker sandbox 默认不改变执行路径；`exec_command` 或 skill API recipe 需要显式请求 `sandbox="docker"` / `"sandbox": {"required": true}`。完整说明见 [`SANDBOX.md`](./SANDBOX.md)。
...
注意：`OPENPPX_SANDBOX_ALLOW_NETWORK` 和 `OPENPPX_SANDBOX_TRUSTED_IMAGES` 是受信任配置上限，不应由模型或动态 recipe 自行生成。
