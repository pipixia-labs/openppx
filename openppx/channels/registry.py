...
def _validate_wecom() -> list[str]:
    ...
    if not WECOM_AVAILABLE:
        issues.append("WeCom channel requires `wecom-aibot-sdk-python` (pip install openppx[wecom]).")
...
def _validate_not_implemented(name: str) -> ChannelValidator:
    def _inner() -> list[str]:
        return [f"Channel '{name}' is recognized but not implemented yet in openppx."]
